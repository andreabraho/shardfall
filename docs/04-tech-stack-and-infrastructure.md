...
`game/Kiln.Game.csproj` (`Godot.NET.Sdk/4.7.2`), `game/project.godot` (`config/features`),
and `.github/workflows/ci.yml` (the `godot-ci` image tag).

**.NET note:** Godot 4.7's `GodotSharp` targets `net8.0`, so `Kiln.Core` and `Kiln.Data`
...
```
Kiln.sln
├─ src/Kiln.Core/          # pure C#, ZERO Godot references
...
├─ src/Kiln.Data/          # JSON loading, schema validation, content registries
├─ src/Kiln.Tests/         # GdUnit4 + xUnit, runs headless in CI
├─ src/Kiln.Tools/         # console apps: balance simulator, data validator, codegen
...
- The **balance simulator** (`Kiln.Tools`) can run 10,000 simulated encounters or walk the
