...
| WLD-05 | **Greybox the 4 field maps** — layout, landmarks, encounter placement, sightlines | **[C→You]** | ◐ Vale Approach (a corridor with a pinch), Vale Floor (a basin with barrows) and the Ridge (the old arena) are built and connected; Broken Gate is the one left. Layouts are a first pass — level design by feel is yours |
